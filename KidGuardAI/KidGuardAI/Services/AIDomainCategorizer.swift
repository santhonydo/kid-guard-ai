...
public class AIDomainCategorizer {
    ...
    public func shouldBlockDomain(_ hostname: String, rules: [Rule]) async throws -> Bool {
        let categories = try await categorizeDomain(hostname)
        
        for rule in rules {
            if rule.actions.contains(.block) && ruleMatchesCategories(rule: rule, categories: categories) {
    ...
    private func ruleMatchesCategories(rule: Rule, categories: [String]) -> Bool {
